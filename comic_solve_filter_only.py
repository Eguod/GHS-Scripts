...
def save_top_level_folder_list_without_zip(top_level_folder_list: list[TopLevelFolder], root_path:str, save_path: str):
    ...
    with tqdm(total=total_pages_number) as pbar:
        for top_level_folder in top_level_folder_list:
            new_top_level_folder_path = os.path.join(save_path, top_level_folder.filename_full)
            if not os.path.exists(new_top_level_folder_path):
                os.mkdir(new_top_level_folder_path)
            for second_level_folder in top_level_folder.sub_folder_list:
                if not os.path.exists(os.path.join(new_top_level_folder_path, second_level_folder.foldername)):
                    os.mkdir(os.path.join(new_top_level_folder_path, second_level_folder.foldername))
                for comic in second_level_folder.content_list:
                    new_comic_path = os.path.join(new_top_level_folder_path, second_level_folder.foldername, comic.filename)
                    ...
                    if not os.path.exists(new_comic_path):
                        os.mkdir(new_comic_path)
                        for file in os.listdir(old_comic_path):
                            shutil.copy(os.path.join(old_comic_path, file), os.path.join(new_comic_path, file))
                            pbar.update(1)
                    else:
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(input("Please input the save path: "), 'output')
    if not os.path.exists(save_path):
    ...
    save_top_level_folder_list_without_zip(top_level_folder_list, root_path, save_path)